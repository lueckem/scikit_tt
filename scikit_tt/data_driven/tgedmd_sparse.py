...
# ideas:
# svd: no orthonormalization to keep sparsity
# check sparsity of M at different steps, keep track of memory
# speed of tensordot very slow. why?
...
def tt_decomposition_reversible(x, basis_list, sigma):
    ...
    cores = [DOK((1, n[0], 1, m * (d + 1)))] + \
            [DOK((m * (d + 1), n[i], 1, m * (d + 1))) for i in range(1, p - 1)] + \
            [DOK((m * (d + 1), n[p - 1], 1, m * d))] + \
            [DOK((m * d, d, 1, m))]

    # insert elements of core 1
    cores[0] = DOK.from_numpy(np.concatenate([dPsix_reversible(basis_list[0], x[:, k], position='first')
                                              for k in range(m)], axis=3))
    ...
    cores.append(DOK.from_numpy(np.eye(m).reshape(m, m, 1, 1)))

    # convert to COO format
    for i in range(len(cores)):
        cores[i] = cores[i].to_coo()
